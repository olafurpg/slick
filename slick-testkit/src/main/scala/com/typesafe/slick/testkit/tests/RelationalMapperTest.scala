...
class RelationalMapperTest extends AsyncTest[RelationalTestDB] {
  ...
  def testMappedType = {
    ...
    implicit val boolTypeMapper: tdb.profile.BaseColumnType[Bool] = MappedColumnType.base[Bool, Int](
      { b =>
        b shouldNotBe null
        if(b == True) 1 else 0
      }, { i =>
  ...
  def testMappedType2 = {
    ...
    implicit val enumTypeMapper: tdb.profile.BaseColumnType[EnumType] = MappedColumnType.base[EnumType, Char](
      { t =>
        t shouldNotBe null
        t match {
          case EnumValue1 => 'A'
  ...
  def testMappedRefType = {
    ...
    implicit val boolTypeMapper: tdb.profile.BaseColumnType[Bool] = MappedColumnType.base[Bool, String](
      { b =>
        b shouldNotBe null
        if(b == True) "y" else "n"
      }, { i =>
