...
object SlickBuild extends Build {
  ...
  def commonSdlcSettings =
    Seq(
        sdlcBase := (projectID.value.name + "-api/")
          .replaceFirst("^slick-", ""),
        sdlcCheckDir :=
        ...
        settings = Defaults.coreDefaultSettings ++ typeProvidersSettings ++ sharedSettings ++ extTarget(
              ...
              scalacOptions in (Compile, doc) <++= version.map(v =>
                    Seq(
                        "-doc-source-url",
                        "https://github.com/slick/slick/blob/" +
                        v + "/slick-testkit/src/main€{FILE_PATH}.scala"
                  )),
        ...
        settings = Defaults.coreDefaultSettings ++ sdlcSettings ++ sharedSettings ++ extTarget(
              ...
              scalacOptions in (Compile, doc) <++= version.map(v =>
                    Seq(
                        "-doc-source-url",
                        "https://github.com/slick/slick/blob/" +
                        v + "/slick-hikaricp/src/main€{FILE_PATH}.scala"
                  )),
     ...
     sourceDirectory in slickProject) map {
      (cp, r, output, s, srcDir, slickSrc) =>
        ...
        val cachedFun = FileFunction.cached(
          ...
          (in: Set[File]) =>
            IO.delete((output ** "*.scala").get)
            toError(r.run("slick.test.codegen.GenerateMainSources",
                          cp.files,
                          Array(outDir),
                          s.log))
            toError(r.run("slick.test.codegen.GenerateRoundtripSources",
                          cp.files,
                          Array(outDir),
                          s.log))
            (output ** "*.scala").get.toSet
