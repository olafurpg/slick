...
import scala.`package`.{ Iterator, Seq }
import slick.ast.{ Node, ProductNode, TypeMapping }
import slick.lifted.{ CaseClassShape, ConstColumn, LiteralColumn, MappedProjection, ProductClassShape, Rep, Shape, ShapedValue, TupleShape }
...
object Shape extends ConstColumnShapeImplicits with AbstractTableShapeImplicits with TupleShapeImplicits {
  implicit final def primitiveShape[T, Level <: ShapeLevel](implicit tm: TypedType[T]): Shape[Level, T, T, ConstColumn[T]] = new Shape[Level, T, T, ConstColumn[T]] {
    def pack(value: Mixed): LiteralColumn[$anon.this.Mixed] = LiteralColumn(value)
    def packedShape: Shape[Level, $anon.this.Packed, $anon.this.Unpacked, $anon.this.Packed] = RepShape[Level, Packed, Unpacked]
    def buildParams(extract: Any => Unpacked): Packed = new ConstColumn[T](new QueryParameter(extract, tm))(tm)
    def encodeRef(value: Mixed, path: Node): Nothing =
  ...
  val unitShapePrototype: Shape[FlatShapeLevel, Unit, Unit, Unit] = new Shape[FlatShapeLevel, Unit, Unit, Unit] {
    ...
    def toNode(value: Mixed): ProductNode = ProductNode(ConstArray.empty)
  }
}
...
object RepShape extends Shape[FlatShapeLevel, Rep[_], Any, Rep[_]] {
  ...
  def encodeRef(value: Mixed, path: Node): slick.lifted.Rep[_] = value.encodeRef(path)
...
abstract class ProductNodeShape[Level <: ShapeLevel, C, M <: C, U <: C, P <: C] extends Shape[Level, M, U, P] {
  ...
  def pack(value: Mixed): ProductNodeShape.this.Packed = {
...
abstract class MappedProductShape[Level <: ShapeLevel, C, M <: C, U <: C, P <: C] extends ProductNodeShape[Level, C, M, U, P] {
  override def toNode(value: Mixed): TypeMapping = TypeMapping(super.toNode(value), MappedScalaType.Mapper(toBase, toMapped, None), classTag)
  def toBase(v: Any): ProductWrapper = new ProductWrapper(getIterator(v.asInstanceOf[C]).toIndexedSeq)
  def toMapped(v: Any): Any = buildValue(TupleSupport.buildIndexedSeq(v.asInstanceOf[Product]))
...
abstract class MappedScalaProductShape[Level <: ShapeLevel, C <: Product, M <: C, U <: C, P <: C](implicit val classTag: ClassTag[U]) extends MappedProductShape[Level, C, M, U, P] {
  override def getIterator(value: C): Iterator[Any] = value.productIterator
  def getElement(value: C, idx: Int): Any = value.productElement(idx)
}
...
final class TupleShape[Level <: ShapeLevel, M <: Product, U <: Product, P <: Product](val shapes: Shape[_ <: ShapeLevel, _, _, _]*) extends ProductNodeShape[Level, Product, M, U, P] {
  override def getIterator(value: Product): Iterator[Any] = value.productIterator
  def getElement(value: Product, idx: Int): Any = value.productElement(idx)
  def buildValue(elems: IndexedSeq[Any]): Product = TupleSupport.buildTuple(elems)
  def copy(shapes: Seq[Shape[_ <: ShapeLevel, _, _, _]]): TupleShape[Level, Nothing, Nothing, Nothing]  = new TupleShape(shapes: _*)
}
...
extends MappedScalaProductShape[FlatShapeLevel, P, LiftedCaseClass, PlainCaseClass, LiftedCaseClass] {
  val shapes: collection.Seq[slick.lifted.Shape[_ <: slick.lifted.ShapeLevel, _, _, _]] = columnShapes.asInstanceOf[TupleShape[_,_,_,_]].shapes
  override def toMapped(v: Any): PlainCaseClass = mapPlain(v.asInstanceOf[PlainTuple])
  def buildValue(elems: IndexedSeq[Any]): LiftedCaseClass = mapLifted(TupleSupport.buildTuple(elems).asInstanceOf[LiftedTuple])
  def copy(s: Seq[Shape[_ <: ShapeLevel, _, _, _]]): CaseClassShape[P, LiftedTuple, LiftedCaseClass, PlainTuple, PlainCaseClass] = new CaseClassShape(mapLifted, mapPlain) { override val shapes: Seq[slick.lifted.Shape[_ <: slick.lifted.ShapeLevel, _, _, _]] = s }
}
...
class ProductClassShape[E <: Product,C <: Product](
  ...
  def buildValue(elems: IndexedSeq[Any]): C = mapLifted(elems)
  def copy(s: Seq[Shape[_ <: ShapeLevel, _, _, _]]): ProductClassShape[E, C] = new ProductClassShape(s, mapLifted, mapPlain)
}
...
case class ShapedValue[T, U](value: T, shape: Shape[_ <: FlatShapeLevel, T, U, _]) extends Rep[U] {
  ...
  def zip[T2, U2](s2: ShapedValue[T2, U2]): ShapedValue[(T, T2), (U, U2)] = new ShapedValue[(T, T2), (U, U2)]((value, s2.value), Shape.tuple2Shape(shape, s2.shape))
  def <>[R : ClassTag](f: (U => R), g: (R => Option[U])): MappedProjection[R, U] = new MappedProjection[R, U](shape.toNode(value), MappedScalaType.Mapper(g.andThen(_.get).asInstanceOf[Any => Any], f.asInstanceOf[Any => Any], None), implicitly[ClassTag[R]])
...
trait ProvenShape[U] {
  ...
  def toNode: Node = packedValue(shape).toNode
}
...
class MappedProjection[T, P](child: Node, mapper: MappedScalaType.Mapper, classTag: ClassTag[T]) extends Rep[T] {
  ...
  def encodeRef(path: Node): MappedProjection[T, P] = new MappedProjection[T, P](child, mapper, classTag) {
    override def toNode: Node = path
  }
  def genericFastPath(f: Function[Any, Any]): MappedProjection[T, P] = new MappedProjection[T, P](child, mapper.copy(fastPath = Some(f)), classTag)
}
