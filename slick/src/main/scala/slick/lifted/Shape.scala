...
abstract class ProductNodeShape[Level <: ShapeLevel, C, M <: C, U <: C, P <: C]
  ...
  def toNode(value: Mixed): Node =
    ProductNode(ConstArray.from(shapes.iterator
...
case class ShapedValue[T, U](
    value: T, shape: Shape[_ <: FlatShapeLevel, T, U, _]) extends Rep[U] {
...
object ShapedValue {
  ...
  def mapToImpl[R <: Product with Serializable, U](c: Context {
    ...
    val fpName = Constant("Fast Path of (" + fields.map(_._2).mkString(", ") +
        ").mapTo[" + rTag.tpe + "]")
