...
final class SingleColumnQueryExtensionMethods[B1, P1, C[_]](
    val q: Query[Rep[P1], _, C]) extends AnyVal {
...
final class AnyOptionExtensionMethods[O <: Rep[_], P](val r: O)
  ...
  def filter[T](p: P => T)(implicit wt: CanBeQueryCondition[T]): O = {
    ...
    r
      .encodeRef(OptionFold(r.toNode, LiteralNode.nullOption, cond, gen))
      .asInstanceOf[O]
  }
...
trait ExtensionMethodConversions {
  ...
  implicit def anyOptionExtensionMethods[T, P](
      v: Rep[Option[T]])(implicit ol: OptionLift[P, Rep[Option[T]]]
