...
trait DatabaseConfig[P <: BasicProfile] {
  ...
  val profile: P
  @deprecated("Use `profile` instead of `driver`", "3.2") val driver: P
...
object StaticDatabaseConfigMacros {
  ...
  def getWithClassLoaderImpl[P <: BasicProfile : c.WeakTypeTag](
      c: Context)(classLoader: c.Expr[ClassLoader])(
      ct: c.Expr[ClassTag[P]]): c.Expr[DatabaseConfig[P]] = {
    import c.universe._
    val uri = c.Expr[String](Literal(Constant(getURI(c))))
