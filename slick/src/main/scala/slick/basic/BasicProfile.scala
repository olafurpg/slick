...
trait BasicProfile extends BasicActionComponent { self: BasicProfile =>
  ...
  trait API extends Aliases with ExtensionMethodConversions {
    ...
    ): ShapedValue[T, U] = new ShapedValue[T, U](value, shape)
...
trait BasicActionComponent { self: BasicProfile =>
  ...
  final type DriverAction [+R, +S <: NoStream,
  -E <: Effect] = ProfileAction[R, S, E]
  @deprecated(
      "Use `StreamingProfileAction` instead of `StreamingDriverAction`", "3.2")
  final type StreamingDriverAction [+R, +T,
  -E <: Effect] = StreamingProfileAction[R, T, E]
