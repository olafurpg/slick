  ...
  def convert(n: Node): Node =
    n match {
      case Join(leftGen, rightGen, left, right, JoinType.Left, on)
        ...
        convert(Union(Join(leftGen, rightGen, left, right, JoinType.Inner, on),
                      Bind(bgen,
                           Filter(lgen2,
                                  assignFreshSymbols(left),
                                  Library.Not.typed(
                                      on.nodeType,
                                      Library.Exists.typed(on.nodeType,
                                                           Filter(rgen2,
                                                                  assignFreshSymbols(
                                                                      right),
                                                                  on2)))),
      ...
      case Join(leftGen, rightGen, left, right, JoinType.Outer, on) =>
        ...
        convert(Union(Join(leftGen, rightGen, left, right, JoinType.Left, on),
                      Bind(bgen,
                           Filter(rgen2,
                                  ...
                                  Library.Not.typed(
                                      ...
                                      Library.Exists.typed(on.nodeType,
                                                           Filter(lgen2,
                                                                  ...
                                                                  on2)))),
                           Pure(ProductNode(ConstArray(
                                       nullStructFor(left.nodeType.structural.asCollectionType.elementType),
                                       Ref(bgen))))),
                      true).infer())
