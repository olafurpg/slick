...
trait JdbcActionComponent extends SqlActionComponent { self: JdbcProfile =>
  ...
  type ProfileAction [+R, +S <: NoStream,
  -E <: Effect] = FixedSqlAction[R, S, E]
  type StreamingProfileAction [+R, +T,
  -E <: Effect] = FixedSqlStreamingAction[R, T, E]
  ...
  class MutatingResultAction[T](rsm: ResultSetMapping,
    ...
    class Mutator(val prit: PositionedResultIterator[T],
                  ...
                  val inv: QueryInvokerImpl[T]) extends ResultSetMutator[T] {
      val pr = prit.pr
      val rs = pr.rs
      var current: T = _
  ...
  protected abstract class InsertActionComposerImpl[U](
      val compiled: CompiledInsert) extends InsertActionComposer[U] {
