...
trait JdbcStatementBuilderComponent { self: JdbcProfile =>
  ...
  class JdbcCompiledInsert(source: Node) {
    ...
    def buildReturnColumns(node: Node
        ): (ConstArray[String], ResultConverter[JdbcResultConverterDomain, _],
    Boolean) = {
  ...
  class QueryBuilder(val tree: Node, val state: CompilerState) {
    ...
    protected def buildForUpdateClause(forUpdate: Boolean) =
      building(OtherPart) {
        if (forUpdate) {
          b"\nfor update "
        }
        ...
        n match {
          ...
          case Library.Substring(n, start, end) =>
            b"\({fn substring($n, ${QueryParameter.constOp[Int]("+")(_ + _)(
                start, LiteralNode(1).infer())}, ${QueryParameter
          ...
          case Apply(sym: Library.SqlFunction, ch) =>
            ...
            b")"
          case n =>
            throw new SlickException("Unexpected function call " + n +
                " -- SQL prefix: " + b.build.sql)
        }
      case c: IfThenElse =>
        b"(case"
      ...
      val (gen, from, where) = tree match {
        case Comprehension(sym,
          ...
          if (fetch.isDefined || offset.isDefined || distinct.isDefined ||
              forUpdate)
            fail(
                ".take, .drop .forUpdate and .distinct are not supported for deleting")
          from match {
  ...
  class TableDDLBuilder(val table: Table[_]) { self =>
    ...
    def buildDDL: DDL = {
      if (primaryKeys.size > 1)
        throw new SlickException("Table " + tableNode.tableName +
      ...
      for (c <- columns) c match {
        case Select(t: TableNode, field: FieldSymbol) =>
          ...
          if (requiredTableName != t.tableName)
            throw new SlickException("All columns in " + typeInfo +
                " must belong to table " + requiredTableName)
        case _ =>
          throw new SlickException("Cannot use column " + c + " in " +
              typeInfo + " (only named columns are allowed)")
      }
    }
  }
