  ...
  class ColumnDDLBuilder(column: FieldSymbol)
    ...
    def createSequenceAndTrigger(t: Table[_]): Iterable[String] =
      if (!autoIncrement) Nil
        ...
        val seq = quoteIdentifier(if (sequenceName eq null)
              t.tableName + "__" + column.name + "_seq" else sequenceName)
        val trg = quoteIdentifier(if (triggerName eq null)
              t.tableName + "__" + column.name + "_trg" else triggerName)
