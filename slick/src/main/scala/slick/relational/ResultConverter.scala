...
final case class ProductResultConverter[M <: ResultConverterDomain, T <: Product](elementConverters: ResultConverter[M, _]*) extends ResultConverter[M, T] {
  ...
  val width: Int = cha.foldLeft(0)(_ + _.width)
  ...
  override def getDumpInfo: DumpInfo = super.getDumpInfo.copy(children = elementConverters.zipWithIndex.map { case (ch, i) => ((i+1).toString, ch) })
}
...
final case class CompoundResultConverter[M <: ResultConverterDomain, @specialized(Byte, Short, Int, Long, Char, Float, Double, Boolean) T](width: Int, childConverters: ResultConverter[M, T]*) extends ResultConverter[M, T] {
  ...
  override def getDumpInfo: DumpInfo = super.getDumpInfo.copy(children = childConverters.zipWithIndex.map {
...
final class GetOrElseResultConverter[M <: ResultConverterDomain, T](child: ResultConverter[M, Option[T]], default: () => T) extends ResultConverter[M, T] {
  def read(pr: Reader): T = child.read(pr).getOrElse(default())
  def update(value: T, pr: Updater): Unit = child.update(Some(value), pr)
  def set(value: T, pp: Writer): Unit = child.set(Some(value), pp)
  def width: Int = child.width
  override def getDumpInfo: DumpInfo =
...
final class IsDefinedResultConverter[M <: ResultConverterDomain](child: ResultConverter[M, Option[_]]) extends ResultConverter[M, Boolean] {
  def read(pr: Reader): Boolean = child.read(pr).isDefined
  def update(value: Boolean, pr: Updater): Nothing =
    throw new SlickException("Cannot insert/update IsDefined check")
  def set(value: Boolean, pp: Writer): Nothing =
    throw new SlickException("Cannot insert/update IsDefined check")
  def width: Int = child.width
  override def getDumpInfo: DumpInfo =
...
final case class TypeMappingResultConverter[M <: ResultConverterDomain, T, C](child: ResultConverter[M, C], toBase: T => C, toMapped: C => T) extends ResultConverter[M, T] {
  def read(pr: Reader): T = toMapped(child.read(pr))
  def update(value: T, pr: Updater): Unit = child.update(toBase(value), pr)
  def set(value: T, pp: Writer): Unit = child.set(toBase(value), pp)
  def width: Int = child.width
  override def getDumpInfo: DumpInfo = super.getDumpInfo.copy(children = Vector(("child", child)))
}
...
final case class OptionRebuildingResultConverter[M <: ResultConverterDomain, T](discriminator: ResultConverter[M, Boolean], data: ResultConverter[M, T]) extends ResultConverter[M, Option[T]] {
  ...
  def set(value: Option[T], pp: Writer): Nothing =
    throw new SlickException("Cannot insert/update non-primitive Option value")
  def width: Int = discriminator.width + data.width
  override def getDumpInfo: DumpInfo = super.getDumpInfo.copy(children = Vector(("discriminator", discriminator), ("data", data)))
}
...
abstract class SimpleFastPathResultConverter[M <: ResultConverterDomain, T](protected[this] val rc: TypeMappingResultConverter[M, T, _]) extends ResultConverter[M, T] {
  ...
  protected[this] def next[C]: ResultConverter[M, C] = {
    idx += 1
    ch(idx).asInstanceOf[ResultConverter[M, C]]
  }

  def read(pr: Reader): T = rc.read(pr)
  def update(value: T, pr: Updater): Unit = rc.update(value, pr)
  def set(value: T, pp: Writer): Unit = rc.set(value, pp)

  override def getDumpInfo: DumpInfo = super.getDumpInfo.copy(name = "SimpleFastPathResultConverter", mainInfo = "", children = Vector(("rc", rc)))
  def width: Int = rc.width
}
